def buy(stock, amount, user, database):
    try:
        ...
        balance_account = database.statement(f'SELECT balance FROM account WHERE id = "{user}"')[0]["balance"]
...
def sell(stock, amount, id, database):
    try:
        ...
        # Update the user's balance
        user_account = database.statement(f'SELECT user FROM user_stock WHERE id = "{id}"')[0]["user"]
        database.statement(f'UPDATE account SET balance = balance + "{amount*sell}" WHERE id = "{ user_account }"')
